...
![protocol](https://img.shields.io/badge/protocol-MCP%20%7C%20MQTT%20%7C%20ESP--NOW%20%7C%20IR%20%7C%20433MHz-orange)
...
├────────────────────────────────────────────────────────────────┤
│  控制中枢   小智终端(MCP工具) + 红外 + 433射频 + MQTT桥          │
│             + ESP-NOW组网 + 离线应急命令                        │
├────────────────────────────────────────────────────────────────┤
│  执行层     智能灯 · 智能插座 · 空调/电视(红外) · 窗帘电机        │
│             · 环境传感器节点 · 摄像头联动                        │
└────────────────────────────────────────────────────────────────┘
```
...
### 🏠 万物互联（IoT）
- **多协议统一控制**：一套语音指令 → 红外(老家电) + 433射频(无线插座) + MQTT(智能生态) + ESP-NOW(自制节点) 四通道全覆盖
- **自制智能节点**：ESP-NOW 组网的智能灯/窗帘/插座/传感器节点，低功耗
- **NAS 知识中枢**：日程播报、文档问答、录音归档、待办管理（RAG）
- **Home Assistant 桥接**：接入上千种智能设备生态
...
├── firmware/
│   ├── terminal/             # 小智终端二次开发（MCP工具/红外/射频/MQTT/ESP-NOW/主动引擎）
│   └── node/                 # 自制智能节点（灯/窗帘/插座/传感器，ESP-NOW从机）
├── gateway/                  # 控制中枢网关（Python，MQTT+红外+射频+场景引擎）
...
| 设备控制 | MCP 工具 + MQTT + ESP-NOW + 红外 + 433MHz |
| 终端硬件 | ESP32-S3（主控）+ 双麦 + 喇叭 + 圆屏 |
| 节点硬件 | ESP32-C3 / ESP32（低功耗从机） |
| 网关 | Python（MQTT Broker 桥接、红外码库、场景引擎） |
